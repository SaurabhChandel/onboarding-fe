import React, { Component } from 'react';
import faceIcon from '../../assets/download.jpeg';
import '/home/nineleaps/work/src/components/sidebar/App.css';
class Sidebar extends Component {
    state = {
        activeSelection : 'Dashboard',
    }

    redirectToSelectedLink = (selection) => {
        this.setState({ activeSelection: selection });
    }

    generateLinks = ( { title = '', sublinks = undefined } ) => {
        const { activeSelection } = this.state;
        let el = null;
        if (title && sublinks ) {
            el = (<div className='links-section'>
                <div className='category'>{title}</div>
                    {   sublinks && sublinks.length ?
                    sublinks.map((d, index) => {
                        return <div
                        className={ activeSelection === d ? 'link active' : 'link inactive' }
                        onClick={() => this.redirectToSelectedLink(d)}>
                            {d}
                        </div>
                    })
                    : null
                    }               
            </div>)
        } else {
            el = <div
            className={ activeSelection === title ? 'link active' : 'link inactive' }
            onClick={() => this.redirectToSelectedLink(title)}>
                {title}
            </div>
        }
        return el;
    }

    getProfileDiv = (personDetails) => {
        return (
        <div className='profile'>
            <div className='image-thumbnail'>
                <img src={personDetails.imageUrl ? personDetails.imageUrl : faceIcon}></img>
            </div>
            <div className='content'>
                <div className='name'>{personDetails.name}</div>
                <div className='user-type'>{personDetails.userType}</div>
            </div>
        </div>
        )
    }

    render() {
        let links = [
            { title: 'Dashboard' },
            { title: 'Employees' },
            { title: 'Batches' },
            { title: 'Manage', sublinks: [ 'Questionnaire Form', 'Task list', 'Role' ] },
            { title: 'More', sublinks: [ 'Blogs', "FAQ's" ] }
        ];
        let personDetails = {
            name: 'Saurabh Chandel',
            userType: 'Super Admin',
            imageUrl: 'data:image/jpeg;base64,'
        };
        return <div className='sidebar'>
            { this.getProfileDiv(personDetails)}
            {
                links.map((d, i) => {
                    return this.generateLinks({...d})
                })
            }
        </div>
    }
}
 
export default Sidebar;